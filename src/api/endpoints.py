...
async def get_entries(stomatology: str = Path(example="adilstom"), date: str = "30.11.2024"):
    print(stomatology)
    try:
...
async def create_entry(form_data: CreateZapisForm, stomatology: str = Path(example="adilstom"), session: AsyncSession = Depends(get_session)):
    print(stomatology)
    try:
...
async def cancel_entry(form_data: DeleteZapisForm, stomatology: str = Path(example="adilstom"), session: AsyncSession = Depends(get_session)):
    form_dict = form_data.model_dump()
